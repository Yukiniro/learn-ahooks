...
初次看到这个 `hook` 时会有一点点懵，因为它的实现方式和我们平时使用 `useState` 的方式有点不一样，它是通过 `useRef` 来保存 `state` 的值，然后通过 `useCallback` 来返回 `state` 的值。
...
<Callout type="error">
    由于实现中的`stateRef.current = state` 发生在 `rendering`，所以当前版本的实现中存在状态的同步问题，如果在 `setState` 之后立即调用 `getState`，那么获取到的值是不对的，因为 `setState` 是异步的，所以需要等待 `setState` 执行完毕之后才能获取到最新的值。
</Callout>
